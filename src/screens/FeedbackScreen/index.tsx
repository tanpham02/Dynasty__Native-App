      ...
      <ScrollView>
        <Box className='flex items-center justify-center mt-2'>
          ...
          <Box className='flex-col self-start px-4 mt-5 w-full'>
            <Text className='font-nunito-500 text-zinc-500'>Nói cho chúng tôi biết trải nghiệm của bạn về món ăn</Text>
            <Select
              bg='#fff'
              borderRadius={8}
              marginTop={2}
            ...
            >
              {feedbackTopics.map((feedbackTopic, index) => (
                <Select.Item key={index} label={feedbackTopic} value={feedbackTopic} className='font-nunito-500' />
              ))}
            </Select>
            <Input
              multiline
            ...
            >
              <Text className={`font-nunito-600 ${currentStar ? 'text-white' : 'text-secondary'}`}>Gửi đánh giá</Text>
            </TouchableOpacity>
          </Box>
        </Box>
      </ScrollView>
